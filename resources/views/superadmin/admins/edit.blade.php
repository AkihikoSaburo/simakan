...
<head>
    ...
    <title>Edit Akun Admin - SIMAKAN</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="bg-brand-snow font-sans text-brand-dark antialiased min-h-screen flex flex-col">

    <x-layout.navbar title="Edit Akun Admin" icon="fa-user-shield" />
    <x-layout.subnav />

    <main class="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">

        <div class="flex items-center gap-3">
            <a href="{{ route('superadmin.admins.index') }}" class="h-9 w-9 rounded-xl bg-white border border-brand-light flex items-center justify-center text-brand-gray hover:text-brand-dark shadow-sm transition-colors">
                <i class="fa-solid fa-arrow-left"></i>
            </a>
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-brand-dark">Edit Admin: {{ $admin->username }}</h2>
                <p class="text-xs text-brand-gray mt-0.5">Ubah informasi akun administrator rumah sakit.</p>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('superadmin.admins.update', $admin->id) }}" class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="password" class="block text-sm font-semibold text-brand-dark">Kata Sandi Baru</label>
                        <span class="text-xs text-brand-gray italic">Biarkan kosong jika tidak ingin diubah</span>
                    </div>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-brand-gray">
                ...
                <div class="pt-4 border-t border-brand-light flex items-center justify-end gap-3">
                    <a href="{{ route('superadmin.admins.index') }}" class="px-5 py-2.5 border border-brand-light text-brand-slate font-bold rounded-xl hover:bg-brand-light/30 transition-colors text-xs cursor-pointer">
                        Batal
                    </a>
