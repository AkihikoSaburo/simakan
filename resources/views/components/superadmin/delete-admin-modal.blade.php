<x-ui.modal name="delete-admin" size="sm" x-data="{ loading: false }">
    ...
    <form id="delete-admin-form" :action="$store.modal.payload?.action" method="POST" @submit="loading=true">
        @csrf
        @method('DELETE')

        <div class="text-center sm:text-left">
            <p class="text-sm text-brand-dark">
                Apakah Anda yakin ingin menghapus akun administrator dengan username 
    ...
    <x-slot:footer>
        ...
        {{-- Gunakan taktik form submission yang aman sebelumnya --}}
