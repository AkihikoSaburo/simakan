...
<body class="bg-brand-snow font-sans text-brand-dark antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div>
            ...
            <p class="text-xs text-brand-gray mt-0.5">Atur parameter dasar aplikasi seperti nama instansi dan zona waktu.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-brand-light shadow-sm p-6">
            <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5">
                ...
                <div>
                    <label for="nama_rumah_sakit" class="block text-sm font-semibold text-brand-dark mb-1.5">Nama Rumah Sakit</label>
                    <div class="relative">
                        ...
                        <input type="text" id="nama_rumah_sakit" name="nama_rumah_sakit" value="{{ old('nama_rumah_sakit', $nama_rumah_sakit) }}" required
                ...
                <div>
                    <label for="timezone" class="block text-sm font-semibold text-brand-dark mb-1.5">Zona Waktu (Timezone)</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-brand-gray">
                        ...
                        <select id="timezone" name="timezone" required
                            ...
                            @foreach($timezones as $tzKey => $tzLabel)
                                <option value="{{ $tzKey }}" {{ old('timezone', $timezone) === $tzKey ? 'selected' : '' }}>{{ $tzLabel }}</option>
                            @endforeach
                        </select>
                    ...
                    <p class="text-[10px] text-brand-gray mt-1.5 leading-relaxed">
                        <i class="fa-solid fa-circle-info"></i> Perubahan zona waktu akan mempengaruhi pencatatan waktu pemesanan makanan pasien dan kalkulasi tanggal hari ini.
                    </p>
                    @error('timezone')
                        <p class="text-rose-600 text-xs mt-1.5 font-medium"><i class="fa-solid fa-triangle-exclamation"></i> {{ $message }}</p>
                    @enderror
