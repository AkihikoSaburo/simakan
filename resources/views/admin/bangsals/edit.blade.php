...
<head>
    ...
    <title>Edit Nama Bangsal - SIMAKAN</title>
...
<body class="bg-brand-snow font-sans text-brand-dark antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-brand-dark">Edit Bangsal: {{ $bangsal->nama_bangsal }}</h2>
                <p class="text-xs text-brand-gray mt-0.5">Perbarui nama bangsal/ruang rawat inap.</p>
            </div>
        </div>

        <div class="bg-white rounded-2xl border border-brand-light shadow-sm p-6">
            <form method="POST" action="{{ route('admin.bangsals.update', $bangsal->id) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="nama_bangsal" name="nama_bangsal" value="{{ old('nama_bangsal', $bangsal->nama_bangsal) }}" required
