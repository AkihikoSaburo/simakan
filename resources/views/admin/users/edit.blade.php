...
<body class="bg-brand-snow font-sans text-brand-dark antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-brand-dark">Edit Akun: {{ $user->username }}</h2>
                <p class="text-xs text-brand-gray mt-0.5">Perbarui informasi akun dapur atau bangsal.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-brand-light shadow-sm p-6">
            <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select id="role" name="role" required onchange="handleRoleChange()"
                            ...
                            <option value="bangsal" {{ old('role', $user->role) === 'bangsal' ? 'selected' : '' }}>Bangsal</option>
                        </select>
                ...
                <div id="bangsal-wrapper" class="{{ old('role', $user->role) === 'bangsal' ? '' : 'hidden' }}">
                    ...
                    <div class="relative">
                        ...
                        <select id="bangsal_id" name="bangsal_id"
                            ...
                            @foreach($bangsals as $bangsal)
                                <option value="{{ $bangsal->id }}" {{ old('bangsal_id', $user->bangsal_id) == $bangsal->id ? 'selected' : '' }}>{{ $bangsal->nama_bangsal }}</option>
                            @endforeach
                        </select>
                        <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-brand-gray">
                            <i class="fa-solid fa-chevron-down text-xs"></i>
